...
Часть отказов изнутри работающего контейнера не лечится: USB-устройство,
захваченное умершим на полуслове драйвером, остаётся занятым сколько ни повторяй
`upsdrvctl`. После `WATCHDOG_MAX_RESTARTS` неудачных попыток контейнер завершается
с ненулевым кодом, и политика перезапуска пересоздаёт его с чистым USB-стеком.
Контейнер, который продолжает падать, виден как `unhealthy` и постоянно
перезапускающийся — это куда лучше, чем отдавать устаревшие данные, пока все
клиенты считают себя защищёнными.

## Страница состояния

`WEB=true` включает read-only страницу на порту 8080:

- `/` — состояние, напряжение на входе и выходе, нагрузка, батарея, температура,
  частота; обновление раз в 5 секунд
- `/cgi-bin/api` — те же данные в JSON
- `/cgi-bin/metrics` — формат Prometheus, готов для Grafana

Стоит 107 КБ (busybox httpd) и **не имеет аутентификации** — держите её в
доверенной сети и не выставляйте в интернет.

Страница показывает только то, что реально отдаёт ваш ИБП. У простых моделей с
протоколом Q1 нет `battery.charge` и `battery.runtime`, поэтому такие плитки
просто отсутствуют, а не выдумываются.
